...
- **Firebase Authentication** を採用（Google Sign-in）
- フロントは Firebase SDK で ID Token を取得
- バックエンドは Firebase Admin SDK で `verifyIdToken` で検証
- ID Token は 1 時間有効、フロントが自動更新

### 12.5.2 認可

- すべてのリソースは `owner_uid` を持つ
- API リクエストの uid と `owner_uid` が一致しないと 403
- MVP では「単一ユーザーが自分のプロジェクトを操作する」のみ
