...
- [CastorOps 設計ドキュメント集](#castorops-設計ドキュメント集)
  - [目次](#目次)
...
本ドキュメントは、ハッカソン提出物 **CastorOps**（GCP上のDevOps AIエージェント）の設計フェーズ確定版である。
...
| 規約事項 | 規約上の要求 | CastorOps の対応 | 判定 |
| --- | --- | --- | --- |
| 必須実行プロダクト | App Engine / Compute Engine / GKE / Cloud Run / Cloud Functions / Cloud TPU/GPU のいずれかを使用 | CastorOps 本体および生成アプリ実行基盤の双方で **Cloud Run** を採用 | ✅ 適合 |
...
> CastorOps は、非エンジニア・個人開発者・少人数開発チームが、GCP 上でアプリケーションを「設計→デプロイ→運用」まで通すための AI エージェントである。コーディング自動化を主役にせず、**クラウド構成の推論・図解・採用理由の説明・コスト/セキュリティ評価・運用可視化** に重心を置く。Codex / Claude Code 系が苦手としてきた「設計と運用のブラックボックス化」を解消することに価値の中心を置く。

「CastorOps」というプロダクト名は、航海の道標として使われてきた星 Castor と、運用を意味する Ops を組み合わせたものである。雑然としたクラウドリソース群から構造と意図を読み取り、ユーザーが進むべき方向を示すことを比喩的に表現している。
...
CastorOps は、**スループットではなく「理解可能性」を最適化目標とする** ことで、この副作用に対処する。
...
これら3者に共通する痛点は「**実装できることと、運用できることの間にあるギャップ**」である。CastorOps はこのギャップを埋める。
...
### 1.4.2 CastorOps の差別化ポイント

| 観点 | 既存 AI コーディングエージェント | CastorOps |
| --- | --- | --- |
| 実装 | ◎ | ○（Gemini で担保、十分） |
| 要件定義 | △ | ◎ 明示的なインタビュー工程 |
| 設計書生成 | △ | ◎ 必須工程に組み込み |
...
CastorOps は **Level 2: ユーザー承認後に実行** を採用する。

| Level | 内容 | CastorOps での扱い |
| --- | --- | --- |
| 1 | 提案のみ、人間が手動実行 | 弱すぎる |
| **2** | **承認後にエージェントが実行** | **採用** |
...
CastorOps は「実装力は十分（Pro 級ではないが Sonnet/Flash 級は確保）+ 可視化は突き抜ける」というポジションを狙う。実装力で Codex/Cursor と真正面から競争はしない。
...
| 用語 | 定義 |
| --- | --- |
| プロジェクト | ユーザーが CastorOps 上で作成する開発単位。1 つのアプリケーションに対応。 |
...
| CastorOps 本体 | ユーザーが利用する CastorOps システム本体（メタアプリ）。 |
| ターゲットアプリ | CastorOps によって生成・デプロイされるユーザーのアプリケーション。 |

「CastorOps 本体」と「ターゲットアプリ」の区別は本書類で繰り返し参照されるため、用語として固定する。
...
**M4（Phase 3 完了 / 6/16）**
- Cloud Run × 2（CastorOps 本体 / ターゲットアプリ）にデプロイできる
...
CastorOps は **2層のアプリケーション** から成る。

1. **CastorOps 本体（メタアプリ）**: ユーザーが対話する CastorOps 自体。Cloud Run で動作。
2. **ターゲットアプリ（生成アプリ）**: CastorOps がユーザー要件から生成し、別 Cloud Run にデプロイするアプリ。
...
flowchart TB
    ...
    subgraph castorops["CastorOps 本体（Cloud Run × 2）"]
...
| Cloud Logging | CastorOps 自身 + ターゲットアプリのログ |
...
Ops Dashboard は **CastorOps の最重要差別化機能** である。
ここが弱ければ、CastorOps は「設計書生成つきコーディングエージェント」に過ぎなくなる。
...
CastorOps のワークフローは **直線フロー + 承認ゲート + チャット再依頼ループ** で構成する。各ゲートはユーザーの明示承認を要求する。
...
**Decision:**
CastorOps 本体（Frontend / Backend）とターゲットアプリの双方で Cloud Run を採用。
...
- CastorOps のコード生成エージェントは **独自実装** する（B-2 (b) 確定）。
...
- 一方、CastorOps の重心は「**AI と対話して構成を変える**」ことにある。GUI 編集は対話エージェントの思想に合致しない。
...
### Scene 3: After / CastorOps 紹介（〜30 秒）

```
画面: CastorOps のロゴ + 一行定義
コピー:
「CastorOps:
クラウドを星座のように読み解く、
...
v1 の 60 秒から **30 秒拡張**。CastorOps の「読み解く」コンセプトをここで完結させる。
...
CastorOps は **「単一オーケストレータ + 複数の役割別エージェント」** の構成を採る。
...
### 9.3.4 Codex 参考のエージェント実行アルゴリズム（CastorOps 版）

Codex は参考実装に留め、CastorOps では Python / FastAPI / Firestore に合わせて独自実装する。ただし、エージェント実行の安全性と観測性に関わる以下のアルゴリズムは Codex の設計を参考に採用する。

#### 採用する考え方

| Codex の考え方 | CastorOps での採用形 |
| --- | --- |
...
```
あなたは CastorOps のサブエージェントです。
CastorOps は GCP 上の DevOps AI エージェントで、設計と運用の理解可能性を最優先します。
...
- ただし完全な GCP 風ではない（CastorOps 固有のアイデンティティを残す）
...
「航路を示す星」というプロダクト名のメタファを、ダークモードに反映できると望ましいが、MVP ではライトモードのみで十分。
...
| バケット名（命名規則） | 用途 |
| --- | --- |
| `co-docs-{project-id}` | 設計書 Markdown のバージョン履歴 |
| `co-arch-{project-id}` | Terraform コード、Architecture Spec JSON |
| `co-snapshots-{project-id}` | 承認スナップショット |
| `co-tf-state-{project-id}` | Terraform State |
| `co-agent-outputs-{project-id}` | エージェント出力アーカイブ |
| `co-logs-archive-{project-id}` | Cloud Logging のアーカイブ（オプション） |
...
| バケット | ルール |
| --- | --- |
| `co-agent-outputs-*` | 90 日後に削除 |
| `co-logs-archive-*` | 30 日後に Coldline、180 日後に削除 |
| その他 | 削除しない（手動管理） |
...
C-1 で確定した「問い合わせ管理 API」を CastorOps が生成するときの、ターゲットアプリ側の API 仕様。
...
CastorOps が生成するターゲットアプリのファイル構成（Python + FastAPI 想定）：
...
CastorOps 本体とターゲットアプリで使う GCP サービスを整理する。

### 13.1.1 CastorOps 本体で使うサービス
...
将来は CastorOps 専用プロジェクトとターゲット専用プロジェクトを分離する。
...
| サービスアカウント | `co-frontend-sa@{proj}.iam.gserviceaccount.com` |
...
| サービスアカウント | `co-backend-sa@{proj}.iam.gserviceaccount.com` |
...
### 13.4.1 CastorOps 本体のビルド

`cloudbuild.yaml`（CastorOps 本体）：
...
トリガー：
- CastorOps の `main` ブランチへの push でフル走行
- Pull Request では lint + test のみ

### 13.4.2 ターゲットアプリのビルド（CastorOps から動的生成）

ターゲットアプリのリポジトリには、CastorOps が `cloudbuild.yaml` を自動生成して commit する。
...
| SA 名 | 用途 | 主な権限 |
| --- | --- | --- |
| `co-frontend-sa` | Frontend Cloud Run | （特になし、静的配信用） |
| `co-backend-sa` | Backend Cloud Run | Vertex AI / Firestore / GCS / Secret 読取 / Logging / **Cloud Build トリガー権限のみ**（リソース作成権限は持たない） |
| `co-cloudbuild-sa` | Cloud Build 専用（v2 強化） | Run Admin / Firestore Admin（DB 作成・読み書き）/ Secret Admin / Artifact Writer / Logging Writer / IAM SA 利用権限 |
| `target-{name}-sa` | ターゲットアプリ Cloud Run | Firestore / Logging / Secret（読み取り）程度 |

**v1 から削除した SA:**
- `co-terraform-sa` — Terraform は不採用（ADR-009 v2）。リソース作成権限は `co-cloudbuild-sa` に集約。
...
`co-backend-sa` の roles（v2 改訂）：

| Role | 理由 |
| --- | --- |
| `roles/aiplatform.user` | Vertex AI 経由で Gemini を呼ぶ |
| `roles/datastore.user` | Firestore 読み書き（CastorOps 本体の状態管理） |
| `roles/storage.objectAdmin` | Cloud Storage 読み書き（`co-*` バケット限定が望ましい） |
...
| `roles/iam.serviceAccountUser` on `co-cloudbuild-sa` | Cloud Build を `co-cloudbuild-sa` として実行する権限 |
...
`co-cloudbuild-sa` の roles（v2 強化）：
...
すべての IAM 関連は CastorOps の初期セットアップスクリプト（`scripts/setup_iam.sh`）で管理する。コンソール手動操作は禁止。
理由：
...
| --- | --- | --- |
| `co-firebase-admin-key` | Firebase Admin SDK の JSON 鍵 | `co-backend-sa` |
| `co-github-oauth-client-secret` | GitHub OAuth Secret | `co-backend-sa` |
| `co-github-user-token-{userId}` | ユーザーごとの OAuth Access Token | `co-backend-sa` |
| `co-gemini-api-key` | （Vertex AI 経由のため不要、開発用フォールバック） | `co-backend-sa` |
| `target-{name}-{varname}` | ターゲットアプリの環境変数 | `target-{name}-sa` |

### 13.6.2 命名規則

- CastorOps 本体：`co-{用途}-{識別子}`
- ターゲット：`target-{name}-{varname}`
- ユーザー固有：`co-{purpose}-{userId}`
...
監査ログは CastorOps の Security セクションで「直近 24 時間の Secret アクセス回数」として要約表示する（**14.5 / 15.6** 参照）。
...
│   └── deploy-self.cloudbuild.yaml            # CastorOps 本体デプロイ用（固定）
├── scripts/
    ...
    async def trigger_apply(self, project_id: str, arch_spec: ArchitectureSpec) -> str:
        ...
        build = self.client.create_build(
            ...
            build={
                ...
                "service_account": f"projects/{project_id}/serviceAccounts/co-cloudbuild-sa@{project_id}.iam.gserviceaccount.com",
...
ハッカソンのデモでは「ユーザーが GCP プロジェクトを準備する → CastorOps がそこにリソースを作る」フローを想定する。
...
```mermaid
flowchart TB
    A[ユーザー: GCPプロジェクト作成] --> B[CastorOps: Settings 画面]
    B --> C[gcloud OAuth で認可]
    C --> D[CastorOps: 必要 API を有効化]
    D --> E[CastorOps: 初期 SA を作成]
    E --> F[CastorOps: Terraform State バケット作成]
...
ハッカソン審査時は、デモ用のサンプル GCP プロジェクトを 1 つ用意し、そこに CastorOps 本体とターゲットアプリの両方を配置する。
これによりセットアップ工程を撮影せずに済む。

## 13.9 コスト試算

### 13.9.1 CastorOps 本体（月額）
...
### 14.1.1 Layer 1: CastorOps 本体の運用

これは「CastorOps 自身が落ちていないか」を見るための監視。提出後の運用を視野に入れる。
...
ユーザーがデプロイしたターゲットアプリを CastorOps の Ops Dashboard で監視する。これがプロダクトの中核価値。
...
CastorOps が生成するターゲットアプリには、共通のログ初期化スニペットを埋め込む。

```python
# generated by CastorOps
...
| Terraform Apply 失敗 | 1 件 | UI 通知（CastorOps 上） |
...
エージェント実行は CastorOps の差別化ポイント。詳細にログを残す。
...
```json
{
  ...
  "loop_iterations": 2,
  "input_snapshot_ref": "gs://co-agent-outputs-proj_abc/runs/run_123/input_snapshot.json",
  "output_uri": "gs://co-agent-outputs-proj_abc/runs/run_123/output.json",
  "phase": "ARCHITECTURE_PROPOSED",
  "project_id": "proj_abc",
  "user_id": "uid_xxx"
}
...
```text
gs://co-agent-outputs-{project}/runs/{run_id}.json
```
...
これにより「ログを CastorOps が読んで Ops Dashboard に要約として表示する」体験が成立する。
...
CastorOps は「**ユーザーの GCP プロジェクトの中で動き、ユーザーの GitHub に Draft PR を作る**」というモデル。
...
| Secret | アクセス可能 SA |
| --- | --- |
| `co-firebase-admin-key` | `co-backend-sa` のみ |
| `co-github-oauth-client-secret` | `co-backend-sa` のみ |
| `co-github-user-token-{userId}` | `co-backend-sa` のみ |
...
H-2 の核となる機能。CastorOps の差別化要素のひとつ。9.7 と同期した実装方針を示す。
...
- ターゲットアプリの SA が CastorOps 本体のリソースにアクセスできないか
...
ユーザーの入力やリポジトリ・ログ・LLM 出力をプロンプトに含める際の、CastorOps 固有のリスク。
...
- ユーザーは CastorOps の Timeline 画面で閲覧
...
- [ ] `templates/inquiry-api/` に CastorOps 専用 Secret が混入していない
...
- CastorOps のデモ用 GCP プロジェクトのリソースは、審査終了後に削除
...
- [ ] `pipelines/deploy-self.cloudbuild.yaml` 作成（CastorOps 本体用、固定）
...
| --- | --- |
| CastorOps 本体 | ユーザーが利用する CastorOps システム本体（メタアプリ） |
| ターゲットアプリ | CastorOps によって生成・デプロイされるユーザーのアプリ |
